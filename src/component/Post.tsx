...
export default function Post({id, imageUrl, title, category, created_at, review_count} : IPost) {
    return(
    <Link to={`/post/${id}`}>
        <Box borderBottom={"1px"} borderBottomColor={"lightgray"}>
            ...
            <Heading width="auto" overflow="hidden" textOverflow="ellipsis" whiteSpace="nowrap" fontSize="sm">
                {title}
            </Heading>
            <Text ml={1} color={"skyblue"}>[{review_count}]</Text>
            </Container>
            <Text color={"dimgray"} ml={20}>{category}</Text>
